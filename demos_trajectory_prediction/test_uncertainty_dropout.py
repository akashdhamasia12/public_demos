...
def trajectories_plot(image, outputs, targets, j, image_agent_all, centroid_, ade, fde, history_traj, maneuver_label, maneuver_pred, output_list, outputs_var_mean, final_index, noise):
    """
...
def trajectories_plot_shift(image, outputs, targets, j, ade, fde, output_list, outputs_var_mean, final_index):
    """
    This function plots the regressed (predicted) keypoints and the actual 
    keypoints for each test image in the batch.
    """
    ...
    ax1.imshow(image[0, 10, :, :], origin='lower', cmap='binary', alpha=0.7)
    ax1.imshow(image[0, 21, :, :], origin='lower', cmap='binary', alpha=0.5)
    ax1.imshow(image[0, 23, :, :], origin='lower', cmap='binary', alpha=0.2)
    ax1.imshow(image[0, 24, :, :], origin='lower', cmap='binary', alpha=0.1)
...
# model.load_state_dict(torch.load(checkpoint['model_state_dict'], map_location=config.DEVICE))
# model.load_state_dict(checkpoint['model_state_dict'], strict=False)#, strict=False) # Error(s) in loading state_dict for ResNet: -> added strict-False
...
def evaluate(model, dataloader, ade_list, fde_list, maneuver_gt, maneuver_outputs, variance_list, f, counter_1, noise):
    ...
    with torch.no_grad():
        ...
        for i, data in enumerate(dataloader):
            ...
            if config.DATASET == "SHIFT":
                ...
                trajectories_plot(image, outputs_mean, keypoints, seq_id.item(), image_agent, centroid_current, ade, fde, history_traj, maneuver_label, maneuver_pred, outputs_list, outputs_var_mean, final_index, noise)

        return ade_list, fde_list, maneuver_gt, maneuver_outputs, variance_list, f, time_list
...
f=open(f"{config.plots}/ade-fde_gpu.txt","w+")
...
noise = False
# validatioon function
ade_list, fde_list, maneuver_gt, maneuver_outputs, variance_list, f, test_time_list = evaluate(model, test_loader_iter, ade_list, fde_list, maneuver_gt, maneuver_outputs, variance_list, f, counter_1, noise)
