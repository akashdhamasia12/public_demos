    ...
    with torch.autograd.profiler.emit_itt():
        with torch.no_grad():
            ...
            for i, data in enumerate(dataloader):
                ...
                with torch.profiler.itt.range('benchmark{}'.format(i)):
                    if mixedprecision:
...
# f=open(f"{config.plots}/ade-fde_queue.txt","w+")

# plot_figures = False
# mixedprecision = False

# # validatioon function
# ade_list, fde_list, test_time_list_no_ipex = evaluate(model, test_loader, ade_list, fde_list, plot_figures, mixedprecision)
...
# f.write(str(sum(test_time_list_no_ipex)/len(test_time_list_no_ipex)) +"\n")
# f.write(str(total_ADE))
...
with torch.autograd.profiler.emit_itt():
    ...
    #         model_ipex = torch.jit.freeze(model_ipex)
            ...
            for x1 in range(20):
                 with torch.profiler.itt.range('warmup_{}'.format(x1)):
...
print("total_ADE(in pixels) =", sum(ade_list)/len(ade_list))
# print("total_FDE(in pixels) =", sum(fde_list)/len(fde_list))
print("total_model_time(s) =", (sum(test_time_list_ipex)/len(test_time_list_ipex))*1000)
